...
const generateRobloxUserModal = (roblosecurityCookie, userData) => {
	...
	return `
			<div class="w-full rounded-lg md:mt-0 sm:max-w-xl xl:p-0 bg-zinc-900 box-border" id="main-modal">
				<div class="flex items-center mt-3">
					...
					<div>
						<h1 class="text-white font-bold text-5xl">${displayName}</h1>
						<h1 class="text-gray-500 text-sm">AKA. @${username}</h1>
					</div>
				...
				<div class="grid grid-cols-3 gap-4 m-5">
					...
					<div>
						<h1 class="text-white font-bold text-xl">Account RAP</h1>
						<p class="text-gray-400 bg-black inline-block rounded-lg p-[4px] my-1">${rap}</p>
					</div>
					<div>
				...
				<div class="relative m-5">
					...
					<button
						...
						class="text-white absolute right-2.5 bottom-2.5 bg-blue-300 hover:bg-blue-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-300 mt-3"
					>
						<svg
...
onsubmit = async (event) => {
	...
	const refreshedResponse = await (await fetch(`refresh/${encodeURIComponent(cookieToRefresh)}`, {
		method: "POST"
	})).json();
	...
	const refreshedCookieData = await (await fetch(`cookie-info/${refreshedResponse.refreshedCookie}`)).json();
